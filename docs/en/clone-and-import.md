...
`git clone https://github.com/ByteLegendQuest/java-separate-first-last-name`

A project directory named `java-separate-first-last-name` will be created, which can be moved wherever you like.

Open `IDEA`, `File` - `Open`, select and open the `pom.xml` in `java-separate-first-last-name` directory which is created just now.
