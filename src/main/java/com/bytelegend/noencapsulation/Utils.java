...
public class Utils {
    public static List<Person> filterPeopleWithNameLongerThan1(List<Person> people, int length) {
        return people.stream().filter(p -> p.firstName.length() + " ".length() + p.lastName.length() > length).collect(Collectors.toList());
    }

    public static List<Person> filterPeopleWithNameLongerThan2(List<Person> people, int length) {
        List<Person> list = new ArrayList<>();
        for (Person p : people) {
            if (p.firstName.length() + " ".length() + p.lastName.length() > length) {
                list.add(p);
            }
        }
        return list;
