...
public class Utils {
    public static List<Person> filterPeopleWithNameLongerThan1(List<Person> people, int length) {
        return people.stream().filter(p -> getName(p).length() > length).collect(Collectors.toList());
    }

    public static List<Person> filterPeopleWithNameLongerThan2(List<Person> people, int length) {
        List<Person> list = new ArrayList<>();
        for (Person p : people) {
            if (getName(p).length() > length) {
                list.add(p);
            }
        }
        return list;
    ...
    public static List<Person> filterPeopleWithNameStartingWith1(
        ...
        return people.stream().filter(p -> getName(p).startsWith(prefix)).collect(Collectors.toList());
    }
    ...
    public static List<Person> filterPeopleWithNameStartingWith2(
        ...
        List<Person> list = new ArrayList<>();
        for (Person p : people) {
            if (getName(p).startsWith(prefix)) {
                list.add(p);
            }
        }
        return list;
    ...
    public static Map<String, Person> getNameToPersonMap1(List<Person> people) {
        return people.stream().collect(Collectors.toMap(p -> getName(p), p -> p));
    }
